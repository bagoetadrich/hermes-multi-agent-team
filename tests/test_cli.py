...
def test_cli_help():
    ...
    assert "hermes-team" in result.output.lower() or "hermes-multi-agent-team" in result.output.lower()
...
def test_cli_lists_all_commands():
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["--help"])
    for cmd in ["init", "configure", "start", "collect-ids", "verify", "status", "doctor", "destroy"]:
